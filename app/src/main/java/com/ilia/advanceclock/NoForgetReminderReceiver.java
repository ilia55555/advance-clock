...
public final class NoForgetReminderReceiver extends BroadcastReceiver {
    @Override public void onReceive(Context context, Intent intent) {
        ...
        Intent open = new Intent(context, NoForgetEditorActivity.class)
                .putExtra("noteId", id)
                .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        PendingIntent content = PendingIntent.getActivity(
                context,
                1_300_000 + (int) Math.abs(id % 1_000_000),
                open,
        ...
        Notification notification = builder
                ...
                .setContentTitle(item.title.trim().isEmpty() ? "NoForget" : item.title)
                .setContentText(item.body.trim().isEmpty() ? "یادآوری شما" : item.body)
        ...
        if (manager != null) manager.notify(NotificationHelper.reminderNotificationId(id), notification);
    }
}
